...
func _on_entrada_transicion_body_entered(body):
	if body.has_method("player"):
		global.transition_to = "entrada_facu"
		global.transition_scene = true

func _on_entrada_transicion_body_exited(body):
	if body.has_method("player"):
		global.transition_scene = false

func _on_patio_transicion_body_entered(body):
	if body.has_method("player"):
		global.transition_to = "patio"
		global.transition_scene = true

func _on_patio_transicion_body_exited(body):
	if body.has_method("player"):
		global.transition_scene = false
