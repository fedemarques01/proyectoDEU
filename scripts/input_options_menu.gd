...
# Store default input actions using physical keycodes
var default_input_mappings = {
	"move_up": KEY_W,
	"move_down": KEY_S,
	"move_left": KEY_A,
	"move_right": KEY_D,
...
func _set_action_texts():
	...
	for i in range(action_buttons.size()):
		...
		button.pressed.connect(Callable(self, "_on_input_button_pressed").bind(button, action))
...
func _on_reset_button_pressed():
	_reset_input_mappings_to_defaults()
	_set_action_texts()

func _reset_input_mappings_to_defaults():
	# Clear existing actions
	for action in input_actions.keys():
		InputMap.action_erase_events(action)

	# Reapply default mappings
	for action in default_input_mappings.keys():
		var event = InputEventKey.new()
		event.physical_keycode = default_input_mappings[action]  # Set using physical_keycode
